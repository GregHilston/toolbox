...
merges the committed base `settings.json` with a per-host overlay. It replaced the old
...
`fetch-thread.py` takes a Hacker News or Reddit URL (or a bare ID) and prints the thread as
markdown or `--format json`. It auto-detects the platform and has zero external
dependencies. Run it with no arguments for usage.
...
See the README for the full flag reference, and `bin/difit.sh` / `bin/git-pr.sh` for the
wrappers themselves.
