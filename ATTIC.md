...
| Audio transcribe/ask | `bin/audio-transcript.py`, `bin/audio-ask.py` | `7ea9e65` | Superseded by `roger/roger/audio/{transcribe,ask}.py` in the home-lab repo. |
| Pixel 8 (Termux) block | `CLAUDE.md` | `657eeac` | Byte-identical duplicate of the block in the global `dot/claude/.claude/CLAUDE.md`. |
...
- **The `enableGui` wording** in `nixos/CLAUDE.md` — flagged as stale, but it is accurate:
  `enableGui` is still a real binding in `modules/home/default.nix`.
