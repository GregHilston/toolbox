...
HARNESS="${AGENT_HARNESS:-artificium}"
if [ "${1:-}" = "--harness" ]; then
  [ $# -ge 2 ] || { echo "--harness needs a value" >&2; exit 64; }
  HARNESS="$2"; shift 2
fi
case "${HARNESS}" in
  artificium|hermes) ;;
  *) echo "unknown harness '${HARNESS}' (artificium|hermes)" >&2; exit 64 ;;
esac
