...
The steps above name four moves generically. Use whichever concrete tool your current harness gives you:
...
| A round of grilling questions | One `AskUserQuestion` call per round, up to 4 questions, the recommended option listed first. A frontier wider than 4 spans consecutive calls. | Numbered questions in the reply, the recommended answer under each, then wait for the typed answers. |
| Spin up a subagent | The `Agent` tool. | The `subagent_*` tools, present only in a session started with `pi-subagents`. Otherwise do the work inline in this session, one ticket at a time, and say so. |
| Tell the user to run `/name` | `/name` | `/skill:name` |
