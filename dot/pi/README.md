# Pi Coding Agent Setup

Local-first AI coding agent ([pi-mono](https://github.com/badlogic/pi-mono)) backed by [oMLX](../omlx/).

## Quick Start

```bash
pi                              # start in current directory
pi --plan                       # start in plan mode (read-only exploration first)
pi @file.png "what's this?"     # vision input (Qwen3.6 supports it)
/plan                           # toggle plan mode mid-session
/todos                          # show plan progress
```

## How It's Deployed

| What | Where | How |
|------|-------|-----|
| `models.json`, `settings.json` | [pi.nix](../../nixos/modules/programs/tui/pi.nix) | Generated by home-manager |
| Local extensions | [dot/pi/.pi/agent/extensions/](../.pi/agent/extensions/) | Stowed to `~/.pi/agent/extensions/` |
| npm packages | [pi.nix](../../nixos/modules/programs/tui/pi.nix) `packages` option, versions locked in [npm/](npm/) | `npm ci` on nix activation, only when the lock changed |
| Sampling params (temp, top_k) | [model_settings.json](../omlx/.omlx/model_settings.json) | oMLX per-model overrides |
| Thinking budget (8192 tokens) | [model_settings.json](../omlx/.omlx/model_settings.json) | Prevents infinite reasoning loops |

## Installed Extensions

**Local** (stow-managed, `dot/pi/.pi/agent/extensions/`) — `ls` it for the
current set. The orchestration ones are inert unless armed, and
[CLAUDE.md](CLAUDE.md) → "Extensions written here for orchestration" explains
what arms each and why it is off by default.

**Packages** are declared in
[pi.nix](../../nixos/modules/programs/tui/pi.nix), which is the list worth
reading — it is the one that deploys. Their versions live in [npm/](npm/),
which Dependabot keeps patched; an eval assertion fails if the two disagree.
To add one, `npm install --save-exact <pkg>` in `npm/`, then list it in pi.nix.

## Watching an unattended run

`/orchestrate-pi` drives workers through `bin/pi-narrate.py` (readable narration
instead of a discarded stream), `bin/pi-workers.py` (a table of every worker's
state) and `bin/pi-rpc.py` (steer one mid-run). See
[CLAUDE.md](CLAUDE.md) → "Seeing what an unattended worker is doing".

## Tips

- **Heavy tools are off until asked for**: `/enable reddit` turns the Reddit
  tools on for this session; `pi-subagents` starts a session with the
  `subagent_*` tools. Decide at the start of a session — enabling mid-way
  re-prefills the whole conversation on oMLX. See [CLAUDE.md](CLAUDE.md) →
  "Lazy tools".
- **Shell output is filtered through `rtk`** before the model sees it
  (`extensions/rtk.ts`, generated by `rtk init --agent pi -g`). `rtk gain` shows
  what it saved; `RTK_DISABLED=1` in the environment switches it off.
- **Thinking loops**: if the model spirals, the 8192-token thinking budget in oMLX will cut it off. You can also try `/nothink` or set `enable_thinking: false` per-request
- **Plan mode for complex tasks**: `/plan` forces the model to read the codebase before touching anything — big win for local models that tend to jump into edits prematurely
- **Keep system prompt lean**: pi's advantage is its tiny system prompt (<1000 tokens). Use `AGENTS.md` in project roots for per-project context, not global system prompt overrides
- **Update packages**: `pi update` pulls latest for all installed packages
