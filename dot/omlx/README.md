...
```
dot/omlx/                    # Shared oMLX config (stowed to ~/.omlx on every Mac)
├── README.md               # This file
├── .gitignore
...
1. **Base config**: `just secrets` (in `nixos/`) renders `settings.json.tpl` →
   `settings.json` via 1Password (`op inject`). Stow links `dot/omlx/.omlx` → `~/.omlx`.
2. **Per-host overlay**: generated by nix from `services.omlxDeploy.cacheSize` in the
   host config (`hosts/macs/<host>/default.nix`). The module
   `nixos/modules/darwin/omlx.nix` deep-merges (`jq -s '.[0] * .[1]'`) the base
   `settings.json` with this overlay into `~/.omlx/settings.json` during
   `darwin-rebuild` activation, then restarts the launchd agent.

`settings.json` itself is excluded from stow (see `.stow-local-ignore`) because it
holds host-specific cache sizes and auth keys; the jq merge is what writes it.
