...
Dotfiles for oMLX LLM inference server, stowed to `~/.omlx`. A per-host settings overlay (cache size, user paths) is generated by nix from `services.omlxDeploy.cacheSize` and jq-merged onto the base `settings.json` during nix-darwin activation. See `README.md` for the deployment flow.
