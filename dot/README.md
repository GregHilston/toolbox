...
```bash
just list              # List the packages the *-all recipes manage
just list-all          # List every directory here, flagging the ones that aren't stowed
...
just stow-all          # Stow every package in the allowlist
just unstow-all        # Unstow every package in the allowlist
just restow-all        # Restow every package in the allowlist
just dry-run aerospace # Preview what stow would do
just dry-run-all       # Preview what stow-all would do
```
...
Run `just list-all` — the justfile is the source of truth, so it can't drift the way a
list here would.

Two directories here are **not** stow packages, and the `*-all` recipes deliberately skip
them (`just list-all` flags both):

- **`claude/`** — deployed by direct symlink instead, by the `link_repo` helper in the root
  `justfile` and by `nixos/modules/programs/tui/claude.nix`.
- **`omlx/`** — deployed by direct `ln` in `nixos/modules/darwin/omlx.nix`. Stowing it
  creates a `toolbox/.omlx` symlink *inside the repo working tree*, because a bare `stow`
  from `dot/` targets the parent directory rather than `$HOME` (see "Stow gotchas" in the
  repo's `CLAUDE.md`).

The `*-all` recipes use an explicit allowlist rather than a `*/` glob, so a package that
stops being used can't keep deploying itself to `$HOME` until someone notices.
