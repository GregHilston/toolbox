...
in {
  flake.nixosConfigurations = {
    # Desktop is opt-in and declared by the host itself via
    # `custom.desktop.enable` (isengard, mines). foundation (WSL) and home-lab
    # (headless Proxmox server) simply don't set it.
    foundation = mkNixos {
      system = "x86_64-linux";
